...
try:
    from candidate_probe import run_candidate_calls
except ModuleNotFoundError:  # Support direct imports from the repository test suite.
    from verifiers.candidate_probe import run_candidate_calls
...
def candidate_identities(
    ...
    def calls(module: str, callable_name: str) -> list[dict[str, Any]]:
        return [
            {
                "module": module,
    ...
    results = run_candidate_calls(
        Path(candidate_root),
        [
            *calls("app.idempotency", "event_identity"),
            *calls("app.search_documents", "document_identity"),
        ],
    )
    split = len(events) * 2

    def checked_pairs(name: str, values: list[Any]) -> list[str]:
        checked: list[str] = []
    ...
    event_keys = checked_pairs("event_key", results[:split])
    document_ids = checked_pairs("document_id", results[split:])
...
def verify_read_only_evidence(settings: Settings) -> list[dict[str, Any]]:
    ...
    with psycopg.connect(dsn) as connection:
        with connection.cursor() as cursor:
            ...
            try:
                cursor.execute("UPDATE evidence.record_evidence SET observed_state = 'tampered'")
...
def publish_events(bootstrap_servers: str, topic: str, events: list[dict[str, Any]]) -> None:
    ...
    try:
        futures = [
            producer.send(topic, key=event["tenant_id"].encode("utf-8"), value=event)
            for event in events
        ]
...
def http_json(method: str, url: str, payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        raise AssertionError(
            f"OpenSearch {method} {url} failed with HTTP {exc.code}: {detail}"
        ) from exc
...
def index_event(
    opensearch_url: str, index: str, doc_id: str, event_key: str, event: dict[str, Any]
) -> None:
    document = {
...
def verify_ledger(connection: psycopg.Connection[Any]) -> list[dict[str, Any]]:
    ...
    if len(rows) != 3:
        raise AssertionError(
            f"idempotency ledger must contain exactly 3 entries, found {len(rows)}"
        )
...
def wait_for_dns(settings: Settings, deadline: float) -> None:
    for host in (
        settings.pg_host,
        settings.kafka_bootstrap_servers.split(":", 1)[0],
        urllib.parse.urlparse(settings.opensearch_url).hostname,
    ):
        if not host:
...
def run() -> dict[str, Any]:
    ...
    identities = candidate_identities(
        settings.candidate_root,
        events,
    )
    identity_by_event = {
        event_fingerprint(event): (
            identities["event_keys"][index],
            identities["document_ids"][index],
        )
    ...
    with psycopg.connect(app_dsn) as app_connection:
        ...
        for record in records:
            event = record.value
            try:
                key, doc_id = identity_by_event[event_fingerprint(event)]
            except KeyError as exc:
                raise AssertionError("Kafka returned an unexpected candidate event") from exc
